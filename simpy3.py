...
base_dir = r'C:\vscode\cremlin\folderhiem'
...
for idx in range(num_images):
    ...
    pil_img = Image.fromarray(radar_img, mode='L')
...
print(f"\nDataset structure:")
print(f"  {dataset_dir}/")
print(f"  ├── images/          ({num_images} grayscale PNG images)")
...
im = ax.imshow(sample_img, cmap='jet', vmin=0, vmax=255, aspect='auto', 
               interpolation='nearest')
cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
cbar.set_label('Intensity (dB)', color='white', fontsize=10)
